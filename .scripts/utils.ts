...
export const getTagName = (version: string): string =>
  `app-v${packageJson.version}`;

/*** helper functions ***/
export function getAssetName(assetPath: string) {
  const basename = path.basename(assetPath);
  const exts = extensionSchema.options
    .map((e) => e.value)
    .filter((s) => basename.includes(s));
...
export function hasDependency(dependencyName: string, root: string): boolean {
  ...
  const isDep = packageJson.dependencies[dependencyName];
  const isDevDep = packageJson.devDependencies[dependencyName];
...
export async function retry<T>(
  ...
  throw new Error("Retry attempts exceeded without success.");
}
